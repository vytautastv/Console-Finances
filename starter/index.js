...
// Pull all unique months from the array

var uniqueMonths = finances.map(entry => entry[0].trim()).filter((month, index, self) => self.indexOf(month) === index);
...
var netTotal = finances.reduce((total, entry) => {
  return total + entry[1];
}, 0);

console.log("Net Total Profit/Losses: $" + netTotal.toFixed(2));

//Average P&L across all months
...
//Biggest increase across all months
...
//Biggest decrease across all months
